...
<head>
  ...
  <style>
    :root{{--paper:#f4f1ea;--surface:#fffdfa;--ink:#18212b;--muted:#5e6b78;--line:#d8d4cb;--accent:#0b7768;--accent-soft:#dff3ee;--chip-ink:#075b50;--chip-line:#9dd3c9;--shadow:0 18px 45px rgba(24,33,43,.09)}}
    @media (prefers-color-scheme: dark){{:root{{--paper:#151a1f;--surface:#1c2228;--ink:#e8e6e1;--muted:#a4adb5;--line:#2c333b;--accent:#4db39d;--accent-soft:rgba(77,179,157,.16);--chip-ink:#9bdccb;--chip-line:#2f6f61;--shadow:0 18px 45px rgba(0,0,0,.45)}}}}
    *{{box-sizing:border-box}} body{{margin:0;background:var(--paper);color:var(--ink);font:16px/1.55 ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}}
    ...
    .preview{{display:flex;aspect-ratio:16/10;align-items:center;justify-content:center;border-bottom:1px solid var(--line);background:linear-gradient(145deg,#edf4f1,#ece7dd);padding:10px}} .preview img{{display:block;width:100%;height:100%;object-fit:contain}}/* preview mat stays light in both themes: transparent art is drawn for light backdrops */
    .card-body{{padding:18px}} .meta{{display:flex;gap:6px;flex-wrap:wrap}} .meta .category{{border-color:var(--chip-line);background:var(--accent-soft);color:var(--chip-ink);font-weight:750}} h2{{margin:.72rem 0 .35rem;font-size:1.24rem;line-height:1.2;letter-spacing:-.018em}} .card p{{min-height:3em;margin:0;color:var(--muted);font-size:.92rem}}
    .card nav{{display:flex;gap:8px;margin-top:16px}} .card nav a{{border-radius:9px;background:var(--ink);color:var(--paper);padding:7px 11px;text-decoration:none;font-size:.82rem;font-weight:700}} .card nav a+ a{{background:transparent;color:var(--ink);box-shadow:inset 0 0 0 1px var(--line)}}
