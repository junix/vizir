...
- 媒介与尺寸：1200 CSS px 宽长图，12 个 SVG 面板 + 页脚，@2x PNG 渲染（chrome-headless-shell CDP，自 y=0 全宽 3600px 定高切片顺序拼接）；零 JS、零外链。【2026-09-06 refine 勘误：原「图片 data-URI 内嵌」作废——面板改为 SVG 原字节内联 `<svg>`（零 `<img>`），章节标题层（kicker/h1/h2/lede/figcaption）迁入 HTML，图内只留图形；正文页面高度 8558 → 8881 CSS px，仍居中 `max-width:1200px`。】
